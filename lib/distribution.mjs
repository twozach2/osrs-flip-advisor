...
// Multi-window trend confirmation: a drift is only trustworthy when the short
// and long lookbacks agree on direction. Splits the series into thirds and
// checks whether each adjacent leg moves the same way as the overall drift,
// returning a [0,1] weight that discounts contradicted (mean-reverting) moves.
function estimateTrendAgreement(weightedLogs, driftPerHour) {
  if (!Number.isFinite(driftPerHour) || driftPerHour === 0) {
    return 0;
  }
  if (weightedLogs.length < 9) {
    return 1;
  }
  ...
  const medians = legs.map((leg) => weightedQuantile(leg, 0.5));
  if (medians.some((value) => value === null)) {
    return 1;
  }

  const direction = Math.sign(driftPerHour);
  const segments = [medians[1] - medians[0], medians[2] - medians[1]];
  const agreeing = segments.filter(
    (delta) => Math.sign(delta) === direction,
  ).length;

  return agreeing / segments.length;
}
...
export function analyzeDistribution(samples = [], options = {}) {
  ...
  if (adaptiveHorizon) {
    ...
    trendAgreement = estimateTrendAgreement(weightedLogs, driftPerHour);
    const rawStrength =
      (Math.abs(driftPerHour) * halfLifeHours) / dispersion.robustSigma;
    trendStrength = clamp(rawStrength - TREND_THRESHOLD, 0, 4) * trendAgreement;
