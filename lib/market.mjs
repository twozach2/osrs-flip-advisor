...
export function buildOpportunity(record, settings, nowSeconds = Date.now() / 1000) {
    ...
    if (solvedDepth === null) {
      ...
      if (solvedDepth < settings.entrySigma - 1e-9) {
        entryBuyPrice = Math.max(
          1,
          Math.round(
            distribution.fairValue * Math.exp(-solvedDepth * distribution.robustSigma),
          ),
        );
      }
    }
  ...
  if (distributionOffers) {
    ...
    projectedFairExit = fairExit * Math.exp(Math.min(0, driftLog) * horizonHours);
  }
  const exitDistanceSigma =
    distributionOffers && driftLog < 0
      ? Math.max(
          0,
          Math.log(offers.sellOffer / Math.max(projectedFairExit, 1)) /
            Math.max(distribution.robustSigma, 1e-9),
        )
      : null;
