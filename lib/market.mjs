...
import { analyzeHistoricalCycles } from "./cycle-analysis.mjs";
...
function rejectOpportunity(diagnostics, reason) {
  if (diagnostics) {
...
function addCyclePattern(opportunity, cyclePattern) {
  const multiplier = Number(cyclePattern?.rankingMultiplier) || 1;
  return {
    ...opportunity,
    cyclePattern,
    patternMultiplier: multiplier,
    patternBalancedScore: opportunity.balancedScore * multiplier,
    patternVolumeScore: opportunity.volumeScore * multiplier,
    patternMarginScore: opportunity.marginScore * multiplier,
    patternHighValueScore: opportunity.highValueScore * multiplier,
  };
}
...
export function buildOpportunity(
  ...
  if (isExcludedItem(item)) {
    return rejectOpportunity(diagnostics, "excluded");
  }
  ...
  if (
    ...
    return rejectOpportunity(
      diagnostics,
      settings.requireDistribution === true && !distribution.available
        ? "history"
        : "exitTarget",
    );
  }
  if (!item?.limit) {
    return rejectOpportunity(diagnostics, "budget");
  }
  ...
  if (risk.score > finiteOr(settings.maxRiskScore, 100)) {
    return rejectOpportunity(diagnostics, "risk");
  }
  ...
  if (quantity < 1) {
    return rejectOpportunity(diagnostics, "riskBudget");
  }
  ...
  if (distribution.available) {
    if (adaptiveEntry) {
      ...
      if (solvedDepth === null) {
        return rejectOpportunity(diagnostics, "entryFill");
      }
  ...
  if (!offers || (settings.requireDistribution === true && !distributionOffers)) {
    return rejectOpportunity(diagnostics, "exitTarget");
  }

  const spreadRatio = offers.rawSpread / offers.buyOffer;

  if (ageMinutes > settings.maxAgeMinutes) {
    return rejectOpportunity(diagnostics, "stale");
  }
  if (offers.profit < settings.minProfit) {
    return rejectOpportunity(diagnostics, "profit");
  }
  if (offers.roi < settings.minRoi) {
    return rejectOpportunity(diagnostics, "roi");
  }
  if (hourlyRoundTrips < settings.minHourlyVolume) {
    return rejectOpportunity(diagnostics, "liquidity");
  }
  if (spreadRatio > settings.maxSpreadRatio) {
    return rejectOpportunity(diagnostics, "spread");
  }
  ...
  if (evPerUnit <= 0) {
    return rejectOpportunity(diagnostics, "negativeEv");
  }
  ...
  const balancedScore =
    ...
    (confidence / 100) *
    riskQuality;
  const highValueFillQuality = distributionOffers
    ? clamp(24 / Math.max(cycleOccupancyHours, 1), 0.1, 1)
    : 0.5;
  const highValueScore =
    Math.log10(offers.buyOffer + 10) *
    Math.log10(offers.profit + 10) *
    Math.log10(expectedCycleProfit + 10) *
    highValueFillQuality *
    (confidence / 100) *
    riskQuality;
...
export function rankOpportunities(records, rawSettings = {}) {
  ...
  const settings = {
    ...
    highValuePriceFloor: Math.max(
      0,
      finiteOr(rawSettings.highValuePriceFloor, 1_000_000),
    ),
  };

  const eligibleRecords = records.filter((record) => !isExcludedItem(record.item));
  const baseOpportunities = eligibleRecords
  ...
  const rankingTime = Date.now() / 1000;
  const baseQualifiedPlan = eligibleRecords
    .map((record) =>
      buildOpportunity(record, planSettings, rankingTime, planRejections),
    )
    .filter(Boolean);
  const highValueSettings = {
    ...planSettings,
    minHourlyVolume: Math.min(planSettings.minHourlyVolume, 1),
    minimumDistributionSamples: Math.min(planSettings.minimumDistributionSamples, 8),
    maxEntryFillHours: Math.max(planSettings.maxEntryFillHours, 24),
  };
  const baseHighValue = eligibleRecords
    .map((record) => buildOpportunity(record, highValueSettings))
    .filter(
      (opportunity) =>
        opportunity && opportunity.buyOffer >= settings.highValuePriceFloor,
    );

  const recordsById = new Map(
    eligibleRecords.map((record) => [record.item.id, record]),
  );
  const cycleCandidateIds = new Set(
    [...baseOpportunities, ...baseQualifiedPlan, ...baseHighValue].map(
      (opportunity) => opportunity.id,
    ),
  );
  const cyclePatterns = new Map();
  for (const id of cycleCandidateIds) {
    ...
    cyclePatterns.set(
      id,
      analyzeHistoricalCycles(record.history, {
        nowSeconds: rankingTime,
        trainingWindowHours: settings.distributionWindowHours,
        halfLifeHours: settings.distributionHalfLifeHours,
        minimumSamples: settings.minimumDistributionSamples,
        entrySigma: settings.entrySigma,
        exitSigma: settings.exitSigma,
        ...
        targetBuilder(distribution) {
          const offers = calculateDistributionOffers(distribution, planSettings);
          return offers
            ? {
                buyOffer: offers.buyOffer,
                sellOffer: offers.sellOffer,
  ...
  const enrich = (opportunity) =>
    addCyclePattern(opportunity, cyclePatterns.get(opportunity.id));
  const opportunities = baseOpportunities.map(enrich);
  const qualifiedPlan = baseQualifiedPlan.map(enrich);
  const highValue = baseHighValue
    .map(enrich)
    .sort(
      (left, right) =>
        right.patternHighValueScore - left.patternHighValueScore ||
        right.expectedCycleProfit - left.expectedCycleProfit ||
  ...
  const plan = qualifiedPlan
    .sort(
      (left, right) =>
        right.patternBalancedScore - left.patternBalancedScore ||
        left.effectiveExitSigma - right.effectiveExitSigma,
    )
    .slice(0, 100);

  const highVolume = [...opportunities]
    .sort((left, right) => right.patternVolumeScore - left.patternVolumeScore)
    .slice(0, 100);
  const highMargin = [...opportunities]
    .sort(
      (left, right) =>
        right.expectedWeeklyProfit * right.patternMultiplier -
          left.expectedWeeklyProfit * left.patternMultiplier ||
        right.patternMarginScore - left.patternMarginScore,
    )
    .slice(0, 100);
  ...
  return {
    ...
    plan,
    plannerDiagnostics: {
      totalMarkets: eligibleRecords.length,
      qualifiedMarkets: qualifiedPlan.length,
      returnedMarkets: plan.length,
      historyWindowHours: settings.distributionWindowHours,
      minimumSamples: settings.minimumDistributionSamples,
      rejected: planRejections,
    },
  };
}
