...
// Closed-form Student-t(df=5) two-tail survival P(|T| > z); df=5 trades a fatter
// tail than Gaussian against still-finite kurtosis, matching event-driven OSRS moves.
function studentTTwoTailSurvival(z) {
  if (z <= 0) {
    return 1;
  }
  ...
  const cdf =
    0.5 +
    (theta + sinTheta * cosTheta * (1 + (2 / 3) * cosSquared)) / Math.PI;
...
function survivalProbability(sigmaDistance) {
  ...
  return clamp(studentTTwoTailSurvival(Math.max(0, sigmaDistance)), 0.02, 0.9);
}
...
export function buildOpportunity(record, settings, nowSeconds = Date.now() / 1000) {
  ...
  const entryPositionBudget = Math.min(
    settings.slotBudget,
    ...
    distribution.available && distribution.fairValue
      ? Math.max(
          ...
          Math.min(
            ...
            Math.floor(entryPositionBudget / Math.max(distribution.fairValue, 1)),
          ),
        )
      : 1;
  ...
  let entryFeasible = true;
  if (adaptiveEntry && distribution.available) {
  ...
  const distributionOffers = entryFeasible
    ? calculateDistributionOffers(distribution, settings, entryBuyPrice)
    : null;
  ...
  const driftLog =
    driftAware && distribution.trendStrength > 0
      ? Number(distribution.driftPerHour) || 0
      : 0;
  const fairExit =
    distribution.available && distribution.fairValue
