...
function highLow(record) {
  const fiveHigh = Number(record.fiveMinute?.avgHighPrice);
  const fiveLow = Number(record.fiveMinute?.avgLowPrice);
  if (fiveHigh > 0 && fiveLow > 0) {
    return { high: fiveHigh, low: fiveLow };
  }
  return {
    high: Number(record.latest?.high) || 0,
    low: Number(record.latest?.low) || 0,
  };
}
...
export class HistoryStore {
  ...
  getTrackedItemIds() {
    return [...this.samples.keys()].map(Number).filter(Number.isFinite);
  }

  getRecentSampleCount(
    itemId,
    windowHours = 72,
    nowSeconds = Math.floor(Date.now() / 1000),
  ) {
    const cutoff = nowSeconds - Math.max(1, Number(windowHours) || 72) * 60 * 60;
    return this.getSamples(itemId).reduce(
      (count, sample) => count + (Number(sample?.[0]) >= cutoff ? 1 : 0),
      0,
    );
  }
      ...
      const snapshot = {
        ...
        d: [
          [
            Number(itemId),
            mid,
            Math.round(spread * 1_000_000),
            Math.min(highVolume, lowVolume),
            Math.round(high),
            Math.round(low),
          ],
        ],
  ...
  addSnapshot(snapshot) {
    for (const entry of snapshot.d || []) {
      ...
      const mid = Number(entry[1]);
      const spread = Number(entry[2]) / 1_000_000;
      let high;
      let low;
      let asymmetric;
      if (entry.length >= 6) {
      ...
      samples.push([snapshot.t, mid, spread, entry[3], high, low, asymmetric]);
